...
router.get("/search", async (req, res, next) => {
  try {
    ...
    if (!r.ok) { res.status(502).json({ error: "Nominatim search failed" }); return; }
...
router.get("/reverse", async (req, res, next) => {
  try {
    ...
    if (!lat || !lng) { res.status(400).json({ error: "Missing lat/lng parameters" }); return; }

    const url = `${NOMINATIM_BASE}/reverse?format=json&addressdetails=1&lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lng)}`;
    const r = await fetch(url, {
      headers: { "User-Agent": UA, "Accept-Language": "en" },
    });
    if (!r.ok) { res.status(502).json({ error: "Nominatim reverse geocode failed" }); return; }
    const data = await r.json();
    res.json(data);
  } catch (err) {
    next(err);
