...
function buildPrintHtml(
  ...
  const eventsHtml = sortedEvents.map((m, i) => {
    ...
    const annotationKey = `${m.eventId}-${i}`;
    const annotation = eventAnnotations[annotationKey]?.trim();
    const annotationHtml = annotation
      ? `<div class="annotation-box"><div class="annotation-label">&#9998; Researcher Annotation</div><p>${escapeHtml(annotation)}</p></div>`
      : "";
    ...
    return `<div class="event-item ${sevClass}">
      ...
      ${idImpact}${reclassImpact}${relevanceNote}${annotationHtml}
    </div>`;
  }).join("\n");
  ...
  const safeResearcherNotes = researcherNotes.trim();
  const researcherNotesHtml = safeResearcherNotes
    ? `<h2>VIII. Researcher Notes &amp; Citations</h2>
       <div class="researcher-notes-box">
         <div class="researcher-notes-label">&#9998; Added by researcher before printing</div>
         <p style="white-space:pre-wrap;">${escapeHtml(safeResearcherNotes)}</p>
       </div>`
    : "";
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .annotation-box { background: #f0f4fb; border: 0.5pt solid #aec4e8; border-radius: 2pt; padding: 5pt 8pt; margin-top: 5pt; font-size: 9.5pt; }
    .annotation-label { font-family: 'Courier New', monospace; font-size: 7pt; text-transform: uppercase; letter-spacing: 0.06em; color: #2a4f8a; margin-bottom: 2pt; }
    .researcher-notes-box { background: #f0f4fb; border: 1pt solid #aec4e8; border-radius: 3pt; padding: 10pt 12pt; font-size: 10.5pt; }
    .researcher-notes-label { font-family: 'Courier New', monospace; font-size: 7.5pt; text-transform: uppercase; letter-spacing: 0.06em; color: #2a4f8a; margin-bottom: 6pt; }
      ...
      <div className="relative bg-white text-zinc-900 w-full max-w-3xl max-h-[92vh] flex flex-col rounded-lg shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          <div ref={reportRef} className="p-8 font-serif text-sm leading-relaxed max-w-2xl mx-auto">
                  ...
                  {sortedEvents.map((match, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Per-event researcher annotation */}
                        <div className="mt-2">
                          <label className="flex items-center gap-1 text-[8.5px] font-mono uppercase tracking-wider text-blue-700 mb-1">
                            <PenLine className="w-2.5 h-2.5" /> Researcher Annotation
                          </label>
                          <textarea
                            rows={2}
                            placeholder="Add a note or primary source citation for this event…"
                            value={eventAnnotations[`${match.eventId}-${i}`] ?? ""}
                            onChange={e =>
                              setEventAnnotations(prev => ({
                                ...prev,
                                [`${match.eventId}-${i}`]: e.target.value,
                              }))
                            }
                            className="w-full text-[10px] font-mono leading-relaxed rounded border border-blue-200 bg-blue-50/50 px-2 py-1.5 text-zinc-800 placeholder:text-zinc-400 focus:outline-none focus:ring-1 focus:ring-blue-400 resize-y"
                          />
                        </div>
                      </div>
                    );
            ...
            {/* ── VIII. Researcher Notes & Citations ── */}
            <div className="mb-7">
              <h2 className="text-xs font-mono font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-200 pb-1.5 mb-4 flex items-center gap-1.5">
              ...
              <textarea
                rows={6}
                placeholder="e.g. Dawes Roll enrollment card located at National Archives, RG 75, Entry 163, Card #12345. Confirms enrollment under Cherokee Nation, circa 1902. Cross-reference with Guion Miller Roll, application #8834…"
                value={researcherNotes}
                onChange={e => setResearcherNotes(e.target.value)}
                className="w-full text-[10.5px] font-mono leading-relaxed rounded border border-blue-200 bg-blue-50/40 px-3 py-2.5 text-zinc-800 placeholder:text-zinc-400 focus:outline-none focus:ring-1 focus:ring-blue-400 resize-y"
              />
            </div>
